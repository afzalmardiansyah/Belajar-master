...
public class UserDAOImpl implements UserDAO {
    ...
    public User save(User param) {
        try {
            String sql = "insert into " + Table.TABEL_USER + " (username, password) value (?, ?)";
    ...
    public User update(User param) {
        try {
            String sql = "update " + Table.TABEL_USER + " set username = ?, password = ? where id = ? ";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, param.getUsername());
            preparedStatement.setString(2, param.getPassword());
    ...
    public int delete(User param) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("delete from " + Table.TABEL_USER + " where id = ?");
    ...
    public User findById(int id) {
        ...
        try {
            PreparedStatement preparedStatement =
                    connection.prepareStatement("SELECT * from " + Table.TABEL_USER + " where id =  ?");
    ...
    public List<User> findAll() {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("SELECT * from " + Table.TABEL_USER);
            while (resultSet.next()) {
                /*User user = new User();
